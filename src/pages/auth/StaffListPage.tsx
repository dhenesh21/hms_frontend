    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <Link to="/admin/staff/new"
          className="flex items-center gap-2 bg-gradient-to-r from-violet-600 to-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:from-violet-700 hover:to-indigo-700 transition shadow-sm shadow-violet-500/30">
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="Search by name, email, ID, role..."
          ...
          className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-violet-500 outline-none" />
      </div>
            ...
            {isLoading ? (
              ...
              <tr>
                <td colSpan={8} className="text-center py-16">
                  ...
                  <Link to="/admin/staff/new" className="text-violet-600 text-sm hover:underline mt-2 inline-block">
                    ...
                    <div className="flex items-center gap-2">
                      {user.photo_url ? (
                        <img src={`http://127.0.0.1:8000${user.photo_url}`} alt={user.full_name}
                          className="w-8 h-8 rounded-full object-cover flex-shrink-0 border border-violet-100" />
                      ) : (
                        <div className="w-8 h-8 bg-violet-100 rounded-full flex items-center justify-center flex-shrink-0">
                          <span className="text-violet-700 text-xs font-bold">{user.full_name?.[0]}</span>
                        </div>
                      )}
