...
import { Shield, Plus, FileText, CheckCircle, Clock, XCircle, AlertCircle, Search } from 'lucide-react'
...
export default function InsurancePage() {
  ...
  const createCompany = useMutation({
    mutationFn: (d: any) => insuranceService.createCompany(cleanPayload(d)),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['ins-companies'] }); companyForm.reset(); toast.success('Company added') },
    onError: (err: any) => toast.error(err.response?.data?.detail || 'Failed to add company')
  })

  const createPolicy = useMutation({
    mutationFn: (d: any) => insuranceService.createPolicy(cleanPayload({ ...d, patient_id: parseInt(d.patient_id), company_id: parseInt(d.company_id), sum_insured: parseFloat(d.sum_insured) })),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['ins-policies'] }); policyForm.reset(); toast.success('Policy added') },
    onError: (err: any) => toast.error(err.response?.data?.detail || 'Failed to add policy')
  })

  const createClaim = useMutation({
    mutationFn: (d: any) => insuranceService.createClaim(cleanPayload({ ...d, policy_id: parseInt(d.policy_id), patient_id: parseInt(d.patient_id), claimed_amount: parseFloat(d.claimed_amount) })),
    ...
    onError: (err: any) => toast.error(err.response?.data?.detail || 'Failed to create claim')
  })
  ...
  const updateClaim = useMutation({
    mutationFn: ({ id, data }: { id: number; data: any }) => insuranceService.updateClaim(id, cleanPayload(data)),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['ins-claims'] }); setSelectedClaim(null); toast.success('Claim updated') },
    onError: (err: any) => toast.error(err.response?.data?.detail || 'Failed to update claim')
  })
    ...
    <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
            ...
            <form onSubmit={companyForm.handleSubmit(d => createCompany.mutate(d))} className="space-y-3">
              ...
              <button type="submit" className="w-full py-2 btn-primary">Add Company</button>
            </form>
          </div>
          <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
            ...
            <form onSubmit={policyForm.handleSubmit(d => {
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="block text-xs text-gray-500 mb-1">Patient ID *</label>
                  <PatientSearchInput inputCls={inputCls} onChange={(id) => policyForm.setValue('patient_id', id)} /></div>
              ...
              <button type="submit" className="w-full py-2 btn-primary">Add Policy</button>
            </form>
          </div>
          <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
      ...
      {tab === 'new-claim' && (
        <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm max-w-2xl">
          ...
          <form onSubmit={claimForm.handleSubmit(d => {
            if (!d.patient_id || isNaN(parseInt(d.patient_id))) return toast.error('Please select a patient')
            createClaim.mutate(d)
          })} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-xs text-gray-500 mb-1">Patient ID *</label>
                <PatientSearchInput inputCls={inputCls} onChange={(id) => claimForm.setValue('patient_id', id)} /></div>
            ...
            <div className="flex justify-end gap-2">
              ...
              <button type="submit" className="px-5 py-2 btn-primary">Create Claim</button>
            </div>
          </form>
        </div>
      )}
