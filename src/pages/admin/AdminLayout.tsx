...
import {
  ...
  Calculator, MoreHorizontal, ChevronLeft, Settings, BedDouble
} from 'lucide-react'
...
const MENU = [
  ...
  },
  {
    group: 'Facilities',
    items: [
      { to: '/admin/facilities', icon: BedDouble, label: 'Wards, Beds & OT' },
    ]
  },
  {
    group: 'HR Management',
      ...
      <aside className="w-56 bg-white border-r border-gray-200 flex flex-col flex-shrink-0 overflow-y-auto">
        <div className="p-4 border-b border-gray-100">
          <button onClick={() => navigate('/dashboard')}
            className="flex items-center gap-2 text-sm text-gray-500 hover:text-violet-600 mb-2">
            <ChevronLeft size={14} /> Back to HMS
          </button>
          <p className="font-bold text-gray-900 text-sm flex items-center gap-2">
            <Settings size={16} className="text-violet-600" /> Admin Panel
          </p>
