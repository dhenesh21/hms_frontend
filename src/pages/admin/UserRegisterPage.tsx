...
const ROLES = [
  { value: 'doctor',         label: 'Doctor',          prefix: 'DR', color: 'bg-violet-100 text-violet-700',    desc: 'OPD, IPD, Prescriptions' },
...
export default function UserRegisterPage() {
  ...
  const { data: users = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        console.log('[Query] Users fetched:', response.data?.length || 0, 'users')
  ...
  const onSubmit = async (data: any) => {
    if (!selectedRole) { 
      ...
      console.log('[Submit] No role selected')
      return 
    }
    
    setLoading(true)
    console.log('[Submit] Starting registration...', { name: data.full_name, email: data.email, role: selectedRole })
    
    try {
      // Step 1: Create user
      console.log('[Submit] Calling API to create user...')
      const res = await authService.createUser({ ...data, role: selectedRole })
      console.log('[Submit] ✅ Full response:', res)
      
      // ✅ Declare userData once here
      const userData = res.data || res
      console.log('[Submit] ✅ User created:', userData?.employee_id, userData?.email)
      
      // Step 2: Invalidate and refetch
      console.log('[Submit] Invalidating users query...')
      try {
        qc.invalidateQueries({ queryKey: ['users'] })
        console.log('[Submit] ✅ Query invalidated')
        
        // Give it a moment to refetch
        await new Promise(resolve => setTimeout(resolve, 500))
        console.log('[Submit] ✅ Waited for refetch')
      } catch (invalidateErr) {
        console.error('[Submit] ⚠️ Error during invalidation:', invalidateErr)
      }
      
      // Step 3: Show success - use userData, don't redeclare
      console.log('[Submit] Setting created state...')
      setCreated({ ...userData, plain_password: data.password })
      console.log('[Submit] ✅ Success! User:', userData?.full_name)
      
      toast.success(`${userData.full_name} registered!`)
      reset()
      setSelectedRole('')
      ...
      console.error('[Submit] Final error message:', msg)
      toast.error(msg)
      
    } finally {
      setLoading(false)
      console.log('[Submit] Done. Loading state reset.')
    }
  }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5">
      <div>
        <h1 className="text-xl font-bold" >Register New Staff</h1>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-2 border-t border-gray-100">
              ...
              <button type="submit" disabled={loading || !selectedRole}
                className="flex items-center gap-2 px-6 py-2 text-sm bg-violet-600 text-white rounded-lg hover:bg-violet-700 disabled:opacity-50 disabled:cursor-not-allowed transition">
                <UserPlus size={16} />
