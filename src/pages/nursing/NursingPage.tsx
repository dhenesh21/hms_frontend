...
import { Pill, ClipboardList, Heart, Users, AlertTriangle, CheckCircle, Plus, Search } from 'lucide-react'
...
export default function NursingPage() {
  ...
  const createMAR = useMutation({
    mutationFn: (d: any) => nursingService.createMAR(cleanPayload({
      ...d, ipd_admission_id: id, patient_id: parseInt(d.patient_id),
      ordered_by: d.ordered_by ? parseInt(d.ordered_by) : undefined,
      start_date: d.start_date,
      scheduled_times: d.scheduled_times?.split(',').map((t: string) => t.trim()).filter(Boolean) || []
    })),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['mar', id] }); marForm.reset(); toast.success('MAR created') },
    onError: (err: any) => toast.error(err.response?.data?.detail || 'Failed to create MAR entry')
  })

  const administerDose = useMutation({
    mutationFn: ({ marId, scheduledTime, status }: any) => nursingService.recordAdministration(cleanPayload({
      mar_id: marId,
    ...
    onError: (err: any) => toast.error(err.response?.data?.detail || 'Failed to record dose')
  })

  const createAssessment = useMutation({
    mutationFn: (d: any) => nursingService.createAssessment(cleanPayload({
      ...
      braden_score: d.braden_score ? parseInt(d.braden_score) : undefined,
    })),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['assessments', id] }); assessForm.reset(); toast.success('Assessment saved') },
    onError: (err: any) => toast.error(err.response?.data?.detail || 'Failed to save assessment')
  })

  const createPlan = useMutation({
    mutationFn: (d: any) => nursingService.createCarePlan(cleanPayload({
      ...d, ipd_admission_id: id,
      patient_id: parseInt(d.patient_id),
    ...
    onError: (err: any) => toast.error(err.response?.data?.detail || 'Failed to create care plan')
  })

  const addIntervention = useMutation({
    mutationFn: (d: any) => nursingService.addIntervention(selectedPlanId!, cleanPayload(d)),
    onSuccess: () => { interventionForm.reset(); setSelectedPlanId(null); toast.success('Intervention recorded') },
    onError: (err: any) => toast.error(err.response?.data?.detail || 'Failed to record intervention')
  })

  const createHandover = useMutation({
    mutationFn: (d: any) => nursingService.createHandover(cleanPayload({
      ...d,
      total_patients: parseInt(d.total_patients || 0),
      critical_patients: parseInt(d.critical_patients || 0),
      new_admissions: parseInt(d.new_admissions || 0),
    ...
    onError: (err: any) => toast.error(err.response?.data?.detail || 'Failed to create handover')
  })
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3 bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
        ...
        <input type="number" value={admissionId} onChange={e => setAdmissionId(e.target.value)}
          className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-violet-500 outline-none w-40"
      ...
      {tab === 0 && (
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
            ...
            <form onSubmit={marForm.handleSubmit(d => {
              if (id <= 0) return toast.error('Please enter an IPD Admission ID')
              if (!d.patient_id || isNaN(parseInt(d.patient_id))) return toast.error('Please select a patient')
              createMAR.mutate(d)
            })} className="grid grid-cols-3 gap-3">
              <div><label className="block text-xs text-gray-500 mb-1">Patient ID *</label>
                <PatientSearchInput inputCls={inputCls} onChange={(id) => marForm.setValue('patient_id', id)} /></div>
              ...
              <div className="col-span-3 flex justify-end">
                <button type="submit" className="px-5 py-2 btn-primary">Add to MAR</button>
              </div>
            </form>
          </div>
              ...
              <div key={dose.administration_id} className="flex items-center gap-4 px-4 py-3">
                ...
                <div className="flex gap-2">
                  <button onClick={() => administerDose.mutate({ marId: dose.mar_id, scheduledTime: `${format(new Date(), 'yyyy-MM-dd')}T${dose.scheduled_time}:00`, status: 'given' })}
                    className="flex items-center gap-1 px-3 py-1.5 btn-primary">
      ...
      {tab === 2 && (
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
            ...
            <form onSubmit={assessForm.handleSubmit(d => {
              if (id <= 0) return toast.error('Please enter an IPD Admission ID')
              if (!d.patient_id || isNaN(parseInt(d.patient_id))) return toast.error('Please select a patient')
              ...
              <div className="grid grid-cols-3 gap-3">
                <div><label className="block text-xs text-gray-500 mb-1">Patient ID *</label>
                  <PatientSearchInput inputCls={inputCls} onChange={(id) => assessForm.setValue('patient_id', id)} /></div>
              ...
              <div className="flex justify-end">
                <button type="submit" className="px-5 py-2 btn-primary">Save Assessment</button>
              </div>
            </form>
          </div>
        ...
        <div className="space-y-4">
          {selectedPlanId && (
            <div className="bg-white rounded-xl border border-blue-200 p-4 shadow-sm">
              ...
              <form onSubmit={interventionForm.handleSubmit(d => addIntervention.mutate(d))} className="space-y-3">
                ...
                <button type="submit" className="px-4 py-2 btn-primary">Save Intervention</button>
              </form>
            </div>
          )}
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
            ...
            <form onSubmit={planForm.handleSubmit(d => {
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-xs text-gray-500 mb-1">Patient ID *</label>
                  <PatientSearchInput inputCls={inputCls} onChange={(id) => planForm.setValue('patient_id', id)} /></div>
                <div><label className="block text-xs text-gray-500 mb-1">Priority</label>
                  <CustomSelect value={planForm.watch('priority') || ''} onChange={v => planForm.setValue('priority', String(v))} placeholder="— Select —" options={[{ value: '', label: '— Select —' }, { value: "high", label: "High" }, { value: "medium", label: "Medium" }, { value: "low", label: "Low" }]} /></div>
              </div>
              ...
              <div className="flex justify-end">
                <button type="submit" className="px-5 py-2 btn-primary">Create Plan</button>
              </div>
            </form>
          </div>
            ...
            <form onSubmit={handoverForm.handleSubmit(d => createHandover.mutate(d))} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="block text-xs text-gray-500 mb-1">From Shift *</label>
                  <CustomSelect value={handoverForm.watch('from_shift') || ''} onChange={v => handoverForm.setValue('from_shift', String(v))} placeholder="— Select —" options={[{ value: '', label: '— Select —' }, { value: "morning", label: "Morning" }, { value: "afternoon", label: "Afternoon" }, { value: "night", label: "Night" }]} /></div>
                <div><label className="block text-xs text-gray-500 mb-1">To Shift *</label>
                  <CustomSelect value={handoverForm.watch('to_shift') || ''} onChange={v => handoverForm.setValue('to_shift', String(v))} placeholder="— Select —" options={[{ value: '', label: '— Select —' }, { value: "afternoon", label: "Afternoon" }, { value: "night", label: "Night" }, { value: "morning", label: "Morning" }]} /></div>
              ...
              <button type="submit" className="w-full py-2 btn-primary">Submit Handover</button>
            </form>
          </div>
                ...
                <div key={h.id} className="border border-gray-100 rounded-xl p-4">
                  ...
                  {!h.received_by && (
                    <button onClick={() => nursingService.receiveHandover(h.id).then(() => { qc.invalidateQueries({ queryKey: ['handovers'] }); toast.success('Handover received') })}
                      className="mt-2 w-full py-1.5 text-xs btn-primary">
