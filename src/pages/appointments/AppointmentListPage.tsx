...
import { appointmentService, notifyService } from '../../services/api'
import { Plus, Calendar, Search, Filter, Bell } from 'lucide-react'
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
        ...
        <div style={{ width: 170 }}>
          <CustomSelect value={statusFilter} onChange={v => setStatusFilter(String(v))} placeholder="All Status"
            options={[{ value: '', label: 'All Status' }, ...Object.entries(STATUS_CONFIG).map(([v, { label }]: any) => ({ value: v, label }))]} />
        </div>
      </div>
                ...
                <td>
                  <div style={{ display: 'flex', gap: 6 }}>
                    ...
                    {(a.status === 'scheduled' || a.status === 'confirmed') && (
                      <button style={{ fontSize: 11, padding: '3px 8px', borderRadius: 6, border: '1px solid #DDD6FE', background: '#F5F3FF', color: '#7C3AED', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 3 }}
                        onClick={() => notifyService.appointment(a.id).then(() => toast.success('Notification sent!')).catch(() => toast.error('Notification failed'))}>
                        <Bell size={11} /> Notify
                      </button>
                    )}
