...
const STATUS_COLORS: Record<string, string> = {
  ...
  pre_op: 'badge-amber',
  in_progress: 'badge-green animate-pulse',
...
const OT_STATUS_COLORS: Record<string, string> = {
  ...
  booked: 'bg-blue-100 border-violet-200',
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: "Today's Surgeries", value: stats?.today_surgeries, color: 'text-violet-600' },
      ...
      {/* OT Status Grid */}
      <div className="card">
        <h2 className="section-title" style={{marginBottom:16}}>Theatre Status</h2>
          ...
          {todaySurgeries?.map((s: any) => (
            <div key={s.id} className="px-5 py-4 flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${STATUS_COLORS[s.status] || 'badge-gray'}`}>
                  {s.status?.replace('_', ' ')}
                </span>
                {s.status === 'scheduled' && (
                ...
                <Link to={`/ot/surgeries/${s.id}`} className="text-xs text-violet-600 hover:underline">
                  Details
                </Link>
              </div>
            </div>
