...
import { ArrowLeft, Thermometer, Heart, Wind, Droplets, Activity, Pill, Printer } from 'lucide-react'
...
export default function OPDVisitDetailPage() {
  ...
  const { data: visit, isLoading } = useQuery({
    ...
    queryFn: () => opdService.getVisit(Number(visitId)).then(r => r.data),
    enabled: !!visitId && !isNaN(Number(visitId))
  })

  if (!visitId || isNaN(Number(visitId))) {
    return <div className="text-center py-20 text-gray-400">Invalid visit ID</div>
  }
    ...
    <div className="max-w-5xl mx-auto space-y-5">
      <div className="flex items-center justify-between gap-3" style={{ marginBottom: 20 }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="page-title">OPD Visit Detail</h1>
            <p className="text-sm text-gray-500">Visit #{visit.visit_number || visit.id}</p>
          </div>
        </div>
        <button
